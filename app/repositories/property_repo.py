...
class PropertyRepository:
    ...
    def get_all_approved(self):
        with session_scope() as session:
            return session.query(SyncedProperty)\
    ...
    def get_by_id(self, property_id: int):
        with session_scope() as session:
            return session.query(SyncedProperty).filter(SyncedProperty.id == property_id).first()
...
class RoomRepository:
    ...
    def get_available(self):
        with session_scope() as session:
            return session.query(SyncedRoom)\
    ...
    def get_by_property(self, property_id: int):
        with session_scope() as session:
            return session.query(SyncedRoom).filter(
    ...
    def get_by_id(self, room_id: int):
        with session_scope() as session:
            return session.query(SyncedRoom).filter(SyncedRoom.id == room_id).first()
...
class SearchPreferenceRepository:
    ...
    def upsert(self, user_id: str, data: dict):
        with session_scope() as session:
            existing = session.query(UserSearchPreference).filter(
...
class RecommendationRepository:
    ...
    def save_property_recommendations(self, user_id: str, results: list):
        with session_scope() as session:
            session.query(PropertyRecommendation).filter(
            ...
            for rank, (prop, score, breakdown) in enumerate(results):
                session.add(PropertyRecommendation(
                    user_id=user_id,
                    property_id=prop.id,
                    score=score,
    ...
    def get_property_recommendations(self, user_id: str):
        with session_scope() as session:
            return session.query(PropertyRecommendation).filter(
                PropertyRecommendation.user_id == user_id
            ).order_by(PropertyRecommendation.rank).all()

    def get_room_recommendations(self, user_id: str):
        with session_scope() as session:
            return session.query(RoomRecommendation).filter(
                RoomRecommendation.user_id == user_id
...
class MatchingRepository:
    ...
    def save_match(self, data: dict):
        with session_scope() as session:
            ...
            existing = session.query(RoommateMatch).filter(
    ...
    def get_matches(self, user_id: str):
        with session_scope() as session:
            return session.query(RoommateMatch).filter(
...
class InteractionRepository:
    ...
    def get_by_user(self, user_id: str, limit: int = None):
        with session_scope() as session:
            q = session.query(UserInteraction).filter(
    ...
    def get_high_dwell(self, user_id: str, min_seconds: int = 10):
        with session_scope() as session:
            return session.query(UserInteraction).filter(
                UserInteraction.user_id == user_id,
